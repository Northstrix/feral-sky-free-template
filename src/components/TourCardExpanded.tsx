...
export default function TourCardExpanded({ id, title, location, description, imageSrc, price, isMobile }: TourCardProps) {
  ...
  return (
    <motion.div
      className="group bg-[#0a0a0a] border border-[#1e1e1e] flex flex-col md:col-span-2 h-full px-1"
    ...
    >
      ...
      <div className="px-5 py-6 flex flex-col gap-6">
        <div 
          ...
          style={{ height: `${imageHeight}px`, borderRadius: '0px' }}
        >
          <img
            src={imageSrc}
            alt={title}
        ...
        <div className={cn("flex-1 flex flex-col gap-4", flexAlignment)}>
          ...
          <p className={cn("text-sm leading-relaxed opacity-60 max-w-3xl text-white", textAlignment)}>
            {description}
          </p>
        </div>
      </div>
