...
const DEFAULTS = {
  ...
  outlineHoverColorDisabled: "#242424",
...
> = ({
  ...
  if (disabled) {
    ...
    borderCol = checkedCoversOutline
      ? (checked ? accentColor : (hovered ? outlineHoverColor : borderColor))
      : (hovered ? outlineHoverColor : borderColor);
  }
  const border =
  ...
  const boxBg = disabled
    ...
    : (checked
        ? (checkedCoversOutline ? accentColor : `linear-gradient(${accentColor} 0 0) padding-box, ${backgroundColor} border-box`)
        : backgroundColor);
